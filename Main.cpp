// Makes window and renders graphics
...
void drawDotProduct(Vector3D a, Vector3D b) {
	...
	// Draw 3 vertices showing dot product
	glLineWidth(2.5);
...
void drawCrossProduct(Vector3D a, Vector3D b) {
	...
	// Draw 3 vertices showing cross product
	glLineWidth(2.5);
	glColor3f(1.0, 0.0, 0.0);
	glBegin(GL_LINES);
	glVertex3f(0.0, 0.0, 0.0);
...
// Keys are pressed and accumulating speed
...
void translateVertices(Vector3D delta) {
	...
	for (int i = 0; i < 24; i++) {
		// Add delta (calculated change) to every vertex of the cube
...
void limitSpeed() {
	...
	// Set to a reasonable speed and stays there at a constant rate
	if (speed < -MAX_SPEED) {
		speed = -MAX_SPEED;
	}
...
void moveCamera(Vector3D delta) {
	...
	// Have camera follow cube
	// Have it go into 3rd and 1st person
...
void move() {
	...
	// Find change in cube position and update it
...
void stop() {
	...
	// If speed is within an absolute error, set it to rest
	if (Math::abs(speed) < 0.1f) {
		speed = 0.f;
	}
	// Else slowly decrement speed until it is at 0
	else if (speed > 0.f) {
		speed -= 0.05f;
	}
